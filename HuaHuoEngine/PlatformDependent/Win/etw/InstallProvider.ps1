...
$resourceFile = "$basedir\UnityETWProvider.dll"
...
$pinfo.Arguments = @("install-manifest", $manifest, "/resourceFilePath:$resourceFile", "/messageFilePath:$resourceFile")
...
If ( $process.ExitCode -ne 0 )
{
    ...
    Write-Error "An error of type $errCode occurred when installing the provider."
    Write-Error $stderr
}
ElseIf ( $stdout -ne "" )
{
...
{
    Write-Output "The Unity ETW provider was installed successfully."    
}
If ($shouldPause)
{
    Pause
