...
void Transform::SetHierarchyCapacity(size_t capacity)
{
    ...
    CopyTransformSubhierarchy(srcHierarchy, 0, *newHierarchy, GetCloneChangeSystemInterestsMask(), TransformChangeSystemMask(0), GetCloneHierarchyChangeSystemInterestsMask(), false);
    ...
    GetTransformHierarchyChangeDispatch().DispatchSelfAndAllChildren(GetRoot().GetTransformAccess(), TransformHierarchyChangeDispatch::kInterestedInTransformAccess);
}
...
void Transform::ValidateHierarchyRecursive(TransformHierarchy& hierarchy, int& hierarchyOrderIndex, int& nextIndex, int parentIndex, UInt8* hasTransformBeenVisited)
{
    ...
    Assert((hierarchy.combinedSystemChanged | hierarchy.systemChanged[index] | gHasChangedDeprecatedSystem.Mask()) == (hierarchy.combinedSystemChanged | gHasChangedDeprecatedSystem.Mask()));
    ...
    for (size_t i = 0; i != m_Children.size(); i++)
        m_Children[i]->ValidateHierarchyRecursive(hierarchy, hierarchyOrderIndex, nextIndex, m_TransformData.index, hasTransformBeenVisited);
...
bool Transform::SetParent(Transform* newFather, SetParentOption options)
{
    ...
    {
        ErrorString(Format("Cannot set the parent of the GameObject '%s' while it is being destroyed.", GetGameObject().GetName()));
        return false;
    }

    if (newFather && newFather->GetGameObject().IsDestroying())
    {
        ErrorString(Format("Cannot set the parent of the GameObject '%s' while its new parent '%s' is being destroyed", GetGameObject().GetName(), newFather->GetGameObject().GetName()));
        return false;
    }
    ...
    if ((GetParent() && GetParent()->GetGameObject().IsActivating()) || (newFather && newFather->GetGameObject().IsActivating()))
    {
        ...
        ErrorString(Format("Cannot set the parent of the GameObject '%s' while activating or deactivating the parent GameObject '%s'.", GetGameObject().GetName(), invalidParent.GetName()));
        return false;
    }
    ...
    if ((options & kAllowParentingFromPrefab) ==  0)
    {
        ...
        {
            ErrorString(Format("Setting the parent of a transform which resides in a Prefab Asset is disabled to prevent data corruption (GameObject: '%s').", GetGameObject().GetName()));
            return false;
        }
    }
...
//        }
    ...
    if (newFather)
    {
        if ((options & kDisableTransformMessage) == 0)
        {
            ...
            if ((previousFather == NULL) || (previousFather->m_TransformData.hierarchy != newFather->m_TransformData.hierarchy))
            {
...
//    }
    ...
    if ((options & kDisableTransformMessage) == 0)
    {
        ...
        if (!previousFather && newFather)
        {
            ...
            AddTransformSubhierarchy(*oldHierarchy, 0, *newHierarchy, newFirst, newLast, GetCloneChangeSystemInterestsMask(), TransformChangeSystemMask(0), GetCloneHierarchyChangeSystemInterestsMask(), false);
        ...
        else if (previousFather && !newFather)
        {
            ...
            CopyTransformSubhierarchy(*oldHierarchy, oldFirst, *newHierarchy, GetCloneChangeSystemInterestsMask(), TransformChangeSystemMask(0), GetCloneHierarchyChangeSystemInterestsMask(), false);
        ...
        else
        {
            ...
            TransformHierarchy* newHierarchy = newFather->m_TransformData.hierarchy;
            UInt32 newFirst = 0, newLast = 0;
            AddTransformSubhierarchy(*oldHierarchy, m_TransformData.index, *newHierarchy, newFirst, newLast, GetCloneChangeSystemInterestsMask(), TransformChangeSystemMask(0), GetCloneHierarchyChangeSystemInterestsMask(), false);
            DetachTransformThread(*oldHierarchy, oldFirst, oldLast);
            UpdateDeepChildCountUpwards(*oldHierarchy, previousFather->m_TransformData.index, -(SInt32)nodeCount);
            FreeTransformThread(*oldHierarchy, oldFirst, oldLast);
            InsertTransformThreadAfter(*newHierarchy, transformThreadInsertIndex, newFirst, newLast);
