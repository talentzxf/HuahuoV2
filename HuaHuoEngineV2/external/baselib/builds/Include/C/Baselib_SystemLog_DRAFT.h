...
BASELIB_API Baselib_SystemLog* Baselib_SystemLog_Open(const char applicationIdentifier[], uint64_t applicationIdentifierLen, const char logIdentifier[], uint32_t logIdentifierLen, Baselib_ErrorState* errorState);
BASELIB_API void Baselib_SystemLog_WriteMessage(Baselib_SystemLog* handle, Baselib_SystemLog_Type type, Baselib_SystemLog_Level level, const char message[], uint32_t messageLen, Baselib_ErrorState* errorState);
