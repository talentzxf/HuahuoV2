...
namespace math
{
    ...
    static MATH_FORCEINLINE float4 quatClamp(const float4& q, float maxAngle)
    {
        float4 ret = q;

        float1 halfCosMaxAnlge = cos(float1(0.5f * maxAngle));
        ...
        if (qn.w < halfCosMaxAnlge)
        {
            ...
            ret = qn * (1.f - fact);
            ret.w = lerp(halfCosMaxAnlge, float1(1.f), fact);
        }
